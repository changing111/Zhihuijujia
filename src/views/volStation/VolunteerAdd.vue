<template >
  <div style="text-align:center;width:1000px;margin-left: 250px;">
    <el-header style="height: 60px;">
        <span style="font-size:30px">义工账号注册</span>&nbsp;&nbsp;&nbsp;&nbsp;
    </el-header>
    <el-main style="height: 700px;text-align:center">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-form-inline" size="big" :inline="true" >
        ...
        <el-form-item label="义工时" prop="valueNum">
                <el-input-number style="width:217px;" v-model="ruleForm.valueNum"  :step="1" :min="0" :max="1000" label="义工时"></el-input-number> 
         </el-form-item>
        <div></div>
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.post(this.Global.basePath+'addVolunteer',{
              ...
              vHour:this.ruleForm.valueNum,
              vVsId:this.ruleForm.vVsId,
              vVsName:this.ruleForm.vVsName,
            })
              .then((res) => {
                if (res.data==1) {
                  this.$alert('恭喜你，注册成功！', '义工账号注册', {
                    confirmButtonText: '确定',
                  });
                  this.resetForm('ruleForm');
                } else {
                  this.$alert('该手机号已存在，请重新提交！', '义工账号注册', {
                    confirmButtonText: '确定',
                  });
                }
              })
