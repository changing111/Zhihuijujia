    ...
    <el-container>
            ...
            <el-container>
              <div id="LoginBackground">
                  ...
                  <h1 style="font-size: 30px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;平台管理员登录</h1>
...
export default {
    ...
    methods: {
      login(data){
        this.axios.post(this.Global.basePath+'loginAdministrator', {
            aId:data.phone,
            aPassword:data.pass
          })
          .then((res) => {
            if (res.data==1) {
              this.$alert('恭喜你，登录成功！', '管理员登录', {
                    confirmButtonText: '确定',
                  });
              sessionStorage.setItem("uId",data.phone);
              sessionStorage.setItem("uAuthor",1);
              this.$router.push({name:'AdminIndex'})
            } else {
              this.$alert('手机号或密码错误，请重新输入！', '管理员登录', {
                    confirmButtonText: '确定',
                  });
            }
          })
