    ...
    <el-header style="height: 100px;white-space: nowrap;  /*强制span不换行*/
      ...
      <div style="height: 50px;">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/VolStationList' }" style="font-size:20px">义工志愿站列表</el-breadcrumb-item>
            <el-breadcrumb-item :to="{ path: '/Volunteer' }" style="font-size:20px">{{this.vsName}}-义工列表</el-breadcrumb-item>
            <el-breadcrumb-item style="font-size:20px">{{this.vName}}-活动列表</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
        <span>&nbsp;&nbsp;活动状态:</span>
  ...
  export default {
    data() {
     return {
        ...
        vPhoneNum:sessionStorage.getItem("vPhoneNum"),
        tableData: [],//定义表格数据
        total:null,    //定义总条数
    ...
    methods: {
      ...
      conditionSerach(){
        this.axios.post('http://localhost:9090/getVolBooksListByVPhoneNum',{
          currentPage:this.currentPage,
          pageSize:this.pageSize,
          volBook:{ 
            vbVPhoneNum:this.vPhoneNum,
      ...
      keywordSerach(){
        this.axios.post('http://localhost:9090/getVolBooksListByVPhoneAndLike',{
          currentPage:this.currentPage,
          pageSize:this.pageSize,
          volBook:{ 
            vbVPhoneNum:this.vPhoneNum,
            vbState:this.inputLike,//把关键字放在vjob中
          }
        })
          .then((res) => {
            this.tableData = res.data.volBooksList
