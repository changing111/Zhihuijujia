  ...
  <div >
    <el-header style="height: 100px;white-space: nowrap;  /*强制span不换行*/
      ...
      <div style="height: 50px;">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item style="font-size:20px">义工志愿站列表</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
        
        <span>&nbsp;&nbsp;所属城市:</span>
        <el-select v-model="valueCity" placeholder="请选择所属城市">
          <el-option
    ...
    <el-main style="height: 640px;">
        <el-table :data="tableData" style="width: 100%" height="600px">
            <el-table-column prop="vsName" label="义工站名称" width="230">
            </el-table-column>
            <el-table-column prop="vsCity" label="所在城市" width="200">
            </el-table-column>
            <el-table-column prop="vsAdminName" label="该站负责人" width="180">
            </el-table-column>
            <el-table-column prop="vsAdminPhone" label="负责人手机号" width="200">
            </el-table-column>
            <el-table-column prop="vsAdminPass" label="负责人账号密码" width="200">
            </el-table-column>
            <el-table-column prop="vsAddress" label="义工站地址" width="250">
            </el-table-column>
            <el-table-column
              fixed="right"
              label="操作"
              ...
              <template slot-scope="scope">
                <el-button @click="routeToVolunteer(scope.row)" type="text" size="big">查看该站义工</el-button>
                <el-button @click="routeToVolActivity(scope.row)" type="text" size="big">查看该站活动</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      conditionSerach(){
        this.axios.post(this.Global.basePath+'getVolStationsListByCondition',{
          currentPage:this.currentPage,
          pageSize:this.pageSize,
          volStation:{ 
            vsCity:this.valueCity,
            vsName:this.inputName,
          }
        })
          .then((res) => {
            this.tableData = res.data.volStationsList
            this.total = res.data.totalCount
          })
      },
      keywordSerach(){
        this.axios.post(this.Global.basePath+'getVolStationsListByInputLike',{
          currentPage:this.currentPage,
          pageSize:this.pageSize,
      ...
      updateClick(row) {  //更新按钮的触发事件
        this.ruleForm.valueCity=row.vsCity,
        this.ruleForm.inputName=row.vsName,
        this.ruleForm.inputHostCard=row.vsHostCard,
        this.ruleForm.inputPhone=row.vsAdminPhone,
        this.ruleForm.inputPass=row.vsAdminPass,
        this.ruleForm.inputAddress=row.vsAddress,
        this.ruleForm.inputHostName=row.vsAdminName,
        this.ruleForm.vsId=row.vsId,
        this.UpdateVisible = true
      },
      deleteClick(row) {  //删除按钮的触发事件
        this.$confirm('此操作将删除该义工站以及其全部义工和活动, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.axios.post(this.Global.basePath+'deleteVolStation',{
              vsId:row.vsId,
            })
              .then((res) => {
                if (res.data==1) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.post(this.Global.basePath+'updateVolStation',{
              ...
              vsId:this.ruleForm.vsId,
            })
              .then((res) => {
                if (res.data==1) {
                  this.$alert('恭喜你，更新成功！', '义工站资料更新', {
                    confirmButtonText: '确定',
                  });
                  //this.resetForm('ruleForm');
                  this.clickChangeSize(this.pageSize);//强行调用分页按钮触发事件来刷新页面
                } else {
                  this.$alert('更新失败！', '义工站资料更新', {
                    confirmButtonText: '确定',
                  });
                }
              })
      ...
      routeToVolunteer(row){
        sessionStorage.setItem("vsName",row.vsName);
        sessionStorage.setItem("vsId",row.vsId);
        this.$router.push({path:'/Volunteer'})
      },
      routeToVolActivity(row){
        sessionStorage.setItem("vsName",row.vsName);
        sessionStorage.setItem("vsId",row.vsId);
        this.$router.push({path:'/VolActivity'})
      },
    },
    components:{
        Pagination
